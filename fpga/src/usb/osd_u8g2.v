/*
    ...
    On-screen display for the FPGA-Companion OSD channel: a 128x64
    monochrome framebuffer laid out like a small OLED panel, which is how
    the u8g2 library on the MCU expects to address it.

    Vendored unchanged from MiSTeryNano (MiSTle-Dev), src/misc/osd_u8g2.v,
    which is the reference implementation of this protocol. GPLv3, same as
    this project.
      https://github.com/MiSTle-Dev/MiSTeryNano

    Protocol, per FPGA-Companion SPI.md and src/osd_u8g2.c:
      command 1 = enable, one data byte, bit0 shows (1) or hides (0)
      command 2 = write, one tile-address byte then N data bytes.
        The address byte is ((y/8)<<4)+(x/8), so bits [6:0] shifted left by
        3 give the byte offset into the 1024-byte buffer, auto-incrementing.
*/
module osd_u8g2 (
  input        clk,
  input        reset,
